...
The scGNN model iterates through these components (Figure 3), enhancing the accuracy of clustering and the reconstruction of gene expression with each pass. Through this iterative refinement, scGNN claims to not only cluster cells effectively but also offer a novel perspective on cellular communication and gene regulatory networks.
...
The visualization of the gene markers on the UMAP (Figure 7) as well as the dot-plot (Figure 8) show that gene markers are confined and specific to particular clusters as expected, indicating an effective clustering.
...
The visualization of the other gene markers on the UMAP (Figure 11), as well as the dot plot (Figure 12), show that some gene markers are relatively confined or specific to particular clusters, while others, such as PPY and GHRL, are not, indicating relatively ineffective clustering compared to the standard approach.
...
The traditional PCA-based method demonstrated robustness and efficiency, managing to effectively identify and segregate the various cell types within the pancreatic islets dataset (Figure 13). The clarity of cluster formation, as visualized in the UMAP plots, and the precise correlation of clusters with known gene markers signify the reliability of this approach. Particularly, the ability of the traditional method to discern even the rare cell types bolsters its utility in scRNA-seq analyses where such granularity is crucial.
